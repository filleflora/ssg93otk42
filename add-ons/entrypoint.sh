...
createAndUpdateMySQLDatabase() {
	...
	if [ "${SSG_DATABASE_TYPE}" == "mysql" ]; then
		...
		LIQUIBASE_JDBC_URL="${LIQUIBASE_JDBC_URL}?autoReconnect=false&characterEncoding=UTF8&characterSetResults=UTF8&socketTimeout=1000&connectTimeout=1000&createDatabaseIfNotExist=true"
...
#Added by Flora to create JDBC/Cassandra connection for OTK installation
prepareOTK(){
	OTK_JDBC_CONNECTION=$(sed -e "s~OTK_DATABASE_USER~${OTK_DATABASE_USER}~g" -e "s~OTK_DATABASE_PASSWORD~${OTK_DATABASE_PASSWORD}~g" -e "s~OTK_DATABASE_JDBC_URL~${OTK_DATABASE_JDBC_URL}~g" /opt/docker/rc.d/base/jdbc-connection.xml)
	echo "${OTK_JDBC_CONNECTION}" > ${GATEWAY_DIR}/node/default/etc/bootstrap/bundle/jdbc-connection.bundle
	
	OTK_CASSANDRA_CONNECTION=$(sed -e "s~OTK_CASSANDRA_KEYSPACE~${OTK_CASSANDRA_KEYSPACE}~g" -e "s~OTK_CASSANDRA_PORT~${OTK_CASSANDRA_PORT}~g" -e "s~OTK_CASSANDRA_CONTACTPOINT~${OTK_CASSANDRA_CONTACTPOINT}~g" /opt/docker/rc.d/base/cassandra-connection.xml)
	echo "${OTK_CASSANDRA_CONNECTION}" > ${GATEWAY_DIR}/node/default/etc/bootstrap/bundle/cassandra-connection.bundle		
}
...
startGateway() {
	...
	eval java \
		...
		>&2 echo "Gateway started. Continuing."
}

#Added by Flora to install OTK
installOTK(){
	...
	if [ $token_endpoint_missing == "1" ]; then
		...
		#call the RESTMAN API to install the solution kit
		...
		eval java \
			...
			>&2 echo "Gateway 9.3 provisioned successfully with OTK 4.2. Done"		
	else
